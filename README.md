...
`lint_figs.py` flags paths crossing text, overlapping text, anything outside the
viewBox, label text wider than the node rect it sits in, and wrapper classes applied to
a leaf element where the CSS only matches them as `.class tag`. It runs in CI and the
deploy fails if it reports anything.
